...
def contourplot(x, y, label, **kwargs):
    euclidean = lambda a, b : np.sqrt(a ** 2 + b ** 2)
    ...
    x_pre, candidate_x_pre = kwargs['x_pre'][:_num_samples], kwargs['x_pre'][_num_samples:]
    y_pre, candidate_y_pre = kwargs['y_pre'][:_num_samples], kwargs['y_pre'][_num_samples:]
    x_post, candidate_x_post = kwargs['x_post'][:_num_samples], kwargs['x_post'][_num_samples:]
    y_post, candidate_y_post = kwargs['y_post'][:_num_samples], kwargs['y_post'][_num_samples:]
...
def draw_graphs(train_data, BO_data, x_post, y_post, cfg, num_iter):
	sns.distplot(train_data[:,0], label='Train data x', color='blue', norm_hist=True)
	sns.distplot(BO_data[:,0], label='Candidate x', color='red', norm_hist=True)
	...
	sns.distplot(train_data[:,1], label='Train data y', color='blue', norm_hist=True)
	sns.distplot(BO_data[:,1], label='Candidate y', color='red', norm_hist=True)
	...
	sns.distplot(x_post[:cfg['num_samples']], label='Train data x', color='blue', norm_hist=False)
	sns.distplot(x_post[cfg['num_samples']:], label='Candidate x', color='red', norm_hist=False)
	...
	sns.distplot(y_post[:cfg['num_samples']], label='Train data y', color='blue', norm_hist=False)
	sns.distplot(x_post[cfg['num_samples']:], label='Candidate y', color='red', norm_hist=False)
