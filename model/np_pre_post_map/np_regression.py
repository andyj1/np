...
class NP(nn.Module):
    def __init__(self, hdim, ddim, z_samples):
        ...
        nn.init.normal_(self.h2.weight)
    ...
    def decoder(self, x_pred, z):
        ...
        decode = self.d2(torch.sigmoid(
            self.d1(x_z).squeeze(-1).transpose(0, 1)))  # 23 x 20 x 2
...
def visualize(x, y, x_star, model, epoch):
    ...
    linecolors = [cm.jet(x) for x in np.random.rand(mu.size(1))]
    for i in range(mu.size(1)):
        ax.plot(x_star.data.cpu().numpy(),
                mu[:, i].data.cpu().numpy(), linewidth=1, label='x_star %d' % i)
        ax.fill_between(
            x_grid[:, 0].data.cpu().numpy(), (mu[:, i] - sigma[:, i]
                                              ).detach().cpu().numpy(),
            (mu[:, i] + sigma[:, i]).detach().cpu().numpy(), alpha=0.2, color=linecolors
        )
        if i == 0:
            ax.scatter(x.data.cpu().numpy(), y.data.cpu().numpy(),
                       color='b', label='context_point %d' % i)
            # ax.plot(all_x_np, all_y_np, color='b', label='all contexts')
    # plt.pause(0.0001)
    ax.legend()
    fig.canvas.draw()
    fig.savefig('regression_result/epoch_'+str(epoch)+'.png')
    # plt.show()

def train(epochs):
    for epoch in range(epochs):
        optimizer.zero_grad()
        inputs = random_split_c_t(
            all_x_np, all_y_np, np.random.randint(20, 30))
        for i in range(len(inputs)):
            inputs[i] = torch.from_numpy(inputs[i]).to(device)
        mu, std, z_mean_all, z_std_all, z_mean, z_std = model(inputs)
        loss = -log_likelihood(mu, std, inputs[1]) + \
        ...
        if epoch % 100 == 0:
            visualize(inputs[0], inputs[1],
                      torch.from_numpy(np.arange(-5, 5, .1).reshape(-1, 1).astype(np.float32)).to(device), model, epoch)

x_grid = torch.from_numpy(
    np.arange(-5, 5, .1).reshape(-1, 1).astype(np.float32))

# create dataset
all_x_np = np.arange(-5, 5, 0.01).reshape(-1, 1).astype(np.float32)
all_y_np = np.sin(all_x_np)
# all_y_np = np.exp(np.cos(all_x_np))**3 * 2*np.sin(all_x_np) - \
#     np.sin(all_x_np)*np.cos(all_x_np)

model = NP(10, 15, 20).to(device)
