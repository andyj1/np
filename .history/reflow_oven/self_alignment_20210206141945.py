    ...
    if args.test == True:
        ...
        if 'y_regressor' in locals():
            ...
            for idx, row in X_test.iterrows():
                ...
                # add text
                ax.annotate(f'({idx}) PRE:{np.linalg.norm((row[0], row[1])):.1f}, SPI:{np.linalg.norm((row[2], row[3])):.1f},VOL:{row[4]:.1f}', xy=(row[0], row[1]), xytext=(row[0]+0.5, row[1]+0.5))
            for idx, row in y_regressor.iterrows():
                ...
                # add text
                ax.annotate(f'({idx}) POST:{np.linalg.norm((row[0], row[1])):.1f}', xy=(row[0], row[1]), xytext=(row[0]+0.5, row[1]+0.5))
