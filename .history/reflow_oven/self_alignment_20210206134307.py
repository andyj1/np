...
def create_self_alignment_model(args):
    ...
    if args.test == True:
        ...
        sampled_chips = customMOM4chipsample(df, input_vars=input_vars, num_samples=args.test_size, chiptype=chip, random_state=SEED)
        X_test = X_test.reshape(sampled_chips.shape)
        print('='*10, f'using a few samples (chip: {chip}, input variables: {input_vars})', '='*10)
        print('X test:', X.shape)

        # Predict on new data
        X_test = X_test.reshape(args.test_size, X.shape[1])
        ...
        if 'y_regressor' in locals():
            ...
            for row in X_test:
                print(row)
            for item in y_regressor:
                print(item)
            sys.exit(0)
            plt.scatter(X_test[:, 0], X_test[:, 1], edgecolor='g', c="yellow", s=s, alpha=a, marker="o",
                        label=f'PRE')
            # POST
            plt.scatter(y_regressor[:, 0], y_regressor[:, 1], edgecolor='k', c="cornflowerblue", s=s, alpha=a+0.3, marker="o",
                        # label=f"regressor (score={regr.score(X_test, y_test):.2f})")
                        label=f'POST')

            plt.xlabel("x")
            plt.ylabel("y")
            plt.title(f"chip: {chip},{regr2name[regressor_type]} regressor({args.num_trees} trees, {args.max_depth} deep) ({args.test_size} samples)")
            plt.legend()
