...
def create_self_alignment_model(args):
    ...
    if args.test == True:
        ...
        y_regressor.reset_index(inplace=True)
        ...
        if 'y_regressor' in locals():
            ...
            for idx, row in X_test.iterrows():
                ...
                # add text
                ax.annotate(f'({idx})', xy=(row[0], row[1]), xytext=(row[0]+1, row[1]+1+idx))
            ...
            for idx, row in y_regressor.iterrows():
                ...
                # add text
                ax.annotate(f'({idx})', xy=(row[0], row[1]), xytext=(row[0]+1, row[1]+offset_y))
                offset_y += 3

            # summary
            for (i1, r1), (i2, r2) in zip(X_test.iterrows(), y_regressor.iterrows()):
                assert i1 == i2
                print(f'({i1}) PRE: {np.linalg.norm((r1[0], r1[1])):.1f}, SPI: {np.linalg.norm((r1[2], r1[3])):.1f}, VOL: {r1[4]:.1f} --> POST: {np.linalg.norm((r2[0], r2[1])):.1f}')
