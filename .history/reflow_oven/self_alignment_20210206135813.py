    ...
    if args.test == True:
        ...
        if 'y_regressor' in locals():
            ...
            for row in X_test.iterrows():
                plt.scatter(row[0], row[1], edgecolor='g', c="yellow", s=s, alpha=a, marker="o", label=f'PRE')
                # add text
                plt.annotate(f'PRE:{np.linalg.norm((row[0], row[1]))}, SPI:{np.linalg.norm((row[2], row[3]))},VOL:{row[4]}', (row[0], row[1])
