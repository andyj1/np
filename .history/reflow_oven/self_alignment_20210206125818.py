...
def customMOM4chipsample(df: pd.DataFrame, input_vars: list, num_samples: int, chiptype: str = None, random_state: int = 42):
    ...
    sampled_chip_df = chip_df.sample(n=num_samples, random_state=random_state)[input_vars]
    
    # make df torch tensor
    flatten = lambda df: torch.FloatTensor(df.to_numpy().reshape(-1,df.shape[1]))
    sampled_chip_df = flatten(sampled_chip_df)
    return sampled_chip_df
...
def parse_args():
    ...
    parser.add_argument('--type', default=1, type=int, help="1: gradient boosting, 2: random forest")
...
def create_self_alignment_model(args):
    ...
    # sampled_chip_df = customMOM4chipsample(df, input_vars=input_vars, num_samples=10, chiptype=chip, random_state=SEED)
    # print(sampled_chip_df)
