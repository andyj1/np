    ...
    if args.test == True:
        ...
        if 'y_regressor' in locals():
            ...
            for idx, row in X_test.iterrows():
                ...
                # add text
                ax.annotate(f'({idx}) PRE:{np.linalg.norm((row[0], row[1])):.1f}, SPI:{np.linalg.norm((row[2], row[3])):.1f},VOL:{row[4]:.1f}', (row[0], row[1]))
            for row in y_regressor:
                ...
                # add text
                ax.annotate(f'({idx}) POST:{np.linalg.norm((row[0], row[1])):.1f}', (row[0], row[1]))
