...
def create_self_alignment_model(args):
    ...
    if args.train:
        ...
        print('='*10,'sizes','='*10)
        print('X train:', X_train.shape,' X test:', X_test.ravel().shape)
        print('Y train:', y_train.shape,' Y test:', y_test.ravel().shape)
    ...
    if args.test == True:
        ...
        if 'y_regressor' in locals():
            ...
            # summary
            result_csv_file = open('reflow_oven/summary.csv', mode='w')
            result_csv_reader = csv.writer(result_csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            result_csv_reader.writerow(['PRE DIR','SPI DIR','PRE-SPI','PRE DIST','SPI DIST','AVG SPI_VOLUME', 'POST DIST'])
            ...
            for (i1, r1), (i2, r2) in zip(X_test.iterrows(), y_regressor.iterrows()):
                ...
                result_csv_reader.writerow([f'({prex_sign}/{prey_sign})', 
