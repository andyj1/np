...
def acq_max(acq_fcn, model, y_max, bounds, random_state, n_warmup=10000, n_iter=10):
    """
    ...
    ----------
    :param acq_fcn:
        The acquisition function object that return its point-wise value.  (function will be available UtilityFunction.utility).

    :param model:
        A gaussian process (or other model) fitted to the relevant data.
    ...
    ys = acq_fcn(x_tries, model=model, y_max=y_max)
    ...
    # print('x seeds:', x_seeds, x_seeds.shape) # shape: (init_points, dimension)
    
    for x_try in x_seeds:
        # Find the minimum of minus the acquisition function
        f0 = lambda x: -acq_fcn(x.reshape(1, -1),model=model, y_max=y_max)        
        res = minimize(f0,
                       x_try,
                       bounds=bounds,
                       method="L-BFGS-B")
...
class UtilityFunction(object):
    ...
    def utility(self, x, model, y_max):
        if self.kind == 'ucb':
            return self._ucb(x, model, self.kappa)
        if self.kind == 'ei':
            return self._ei(x, model, y_max, self.xi)
        if self.kind == 'poi':
            return self._poi(x, model, y_max, self.xi)
