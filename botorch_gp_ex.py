...
if __name__ == '__main__':
	...
	print('[PRE]:',type(inputs), inputs.shape)
	print('[POST DISTANCE]:',type(outputs), outputs.shape)
	...
	iteration = 0
	# if inputs.shape[1] == 2:
	ax = plot_grid(fig, ax, inputs[:, 0], inputs[:, 1], bound_min, bound_max, save_image_path, iteration)
	plt.colorbar()
	plt.pause(0.001)
	iteration += 1 
	...
	for t_iter in range(test_iter):
		...
		dim = inputs.shape[1]
		bounds = torch.stack([torch.ones(dim) * bound_min, torch.ones(dim) * bound_max])
		...
		ax = plot_grid(fig, ax, inputs[:, 0], inputs[:, 1], bound_min, bound_max, save_image_path, iteration)
		plt.legend()
		plt.title(f'{num_dim}-D GPR: minimize POST distance')
		plt.pause(0.001)
		iteration += 1
